    ...
    for ( i = 0, l = children.length; i < l; i += 1 ) {
        ...
        if ( nodeType === ELEMENT_NODE ) {
            ...
            if ( nodeType === TEXT_NODE ) {
                data = child.data;
                startsWithWS = !notWSCRLF.test( data.charAt( 0 ) );
                 endsWithWS = !notWSCRLF.test( data.charAt( data.length - 1 ) );
...
var onPaste = function ( event ) {
 ...
 if ( clipboardData && (
                        indexOf.call( clipboardData.types, 'image/png' ) > -1 || (
                                                                                  indexOf.call( clipboardData.types, 'image/tiff' ) > -1))) {
 // strip single image paste from clipboard - Safari
 event.preventDefault();
 return;
 }
	}
...
proto.setHTML = function ( html ) {
    ...
    cleanTree( frag );
    // cleanupBRs( frag, root ); //this line removes line breaks at the end
